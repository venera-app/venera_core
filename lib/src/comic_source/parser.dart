...
class ComicSourceParser {
  ...
  Future<ComicSource> createAndParse(String js, String fileName) async {
    ...
    final dir = Directory(FilePath.join(_Config.dataPath, "comic_source"));
    if (!dir.existsSync()) {
      dir.createSync(recursive: true);
    }
    var file = File(FilePath.join(dir.path, fileName));
    if (file.existsSync()) {
      int i = 0;
      while (file.existsSync()) {
        file = File(
          FilePath.join(dir.path, "${fileName.split('.').first}($i).js"),
        );
        i++;
      }
    }
